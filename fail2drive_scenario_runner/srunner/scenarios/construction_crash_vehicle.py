...
class ConstructionObstacle(BasicScenario):
    ...
    def _spawn_side_prop(self, wp):
        ...
        while True:
            i += 1
            if i > 100:
                self.other_actors.append(None) # Hack fix
                return
...
class CustomConstructionObstacle(ConstructionObstacle):
    """ Modified version of ConstructionObstacle only used as parent for custom scenarios"""
    ...
    def _initialize_actors(self, config):
        ...
        if "Passable" in config.name:
            return

        #sometimes ConstructionObstacle and ConstructionObstacleTwoWays are wrong labeled, hence distinguish them here
        side_lane_wp = self._construction_wp.get_left_lane() if self._direction == 'right' else self._construction_wp.get_right_lane()
        scenario_name = 'ConstructionObstacle' if side_lane_wp.lane_id * self._construction_wp.lane_id > 0 else 'ConstructionObstacleTwoWays'
        CarlaDataProvider.active_scenarios.append((scenario_name, [self.first, self.last, self._direction, False, 1e9, 1e9, False])) # added

class CustomConstructionObstacleTwoWays(CustomConstructionObstacle):
    """
    Version of ConstructionObstacleTwoWays that inherits CustomConstructionObstacle, used as parent for custom scenarios
    """
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=60):
...
class AlteredConstructionObstacle(CustomConstructionObstacle):
    ...
    def _create_construction_setup(self, start_transform, lane_width):
        ...
        # Traffic warning and debris
        for key, value in _initial_offset.items():
            if key == 'cones' or _prop_names[key] == "none":
                continue
            transform = carla.Transform(
                start_transform.location,
                start_transform.rotation)
        ...
        # Prevent cones from being placed during the scenario according to config
        cones_config = get_value_parameter(self.config, 'cones', str, '1111111')
        self._construction_transforms[-7:] = [x for x, y in zip(self._construction_transforms[-7:], cones_config) if y=="1"]
...
class CustomObstacle(CustomConstructionObstacle):
...
class CustomObstacleTwoWays(CustomConstructionObstacleTwoWays):
    ...
    def _create_construction_setup(self, start_transform, lane_width):
        ...
        # statics = [("static.prop.bin", 1, 0, 30), ("static.prop.bin", 2, 0, 30), ("static.prop.bin", -1, 0, 30)]
        actors = []
...
class ConstructionObstaclePedestrian(CustomConstructionObstacle):
    """
    This class holds everything required for a construction scenario
    The ego vehicle is passing through a road and encounters
    a stationary rectangular construction cones setup and traffic warning,
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False,
        ...
        self.pedestrian_bp = get_value_parameter(config, 'pedestrian', str, 'walker.pedestrian.0030')
    ...
    def _initialize_actors(self, config):
        ...
        self._adversary_transform = self._construction_wp.transform
        yaw = 90 if self._direction == "left" else 270
        self._adversary_transform.rotation.yaw += yaw
        # Den adversary transform einfach vom warning sign abhängig machen!
        self._collision_wp = self._construction_wp.get_left_lane() if self._direction == 'right' else self._construction_wp.get_right_lane()

        self.adversary = CarlaDataProvider.request_new_actor(self.pedestrian_bp, self._adversary_transform)
        if self.adversary is None:
            raise Exception("Couldn't spawn the pedestrian actor")

        self._adversary_transform = self.adversary.get_transform()

        self.adversary.set_location(self._adversary_transform.location + carla.Location(z=-200))
        self.adversary = self._replace_walker(self.adversary)

        self._construction_transforms.append([self.adversary, self._adversary_transform])

    def _create_behavior(self):

        self._max_speed = 1 # TODO es hat glaub was mit dem SetMaxSpeed zu tun
        root = super()._create_behavior()

        root.insert_child(ChangeRoadBehavior(0,0,200,200), 0)

        sequence = py_trees.composites.Sequence(name="ConstructionPedestrian")
        ...
        # insert in parallel behavior
        for x in root.children:
            if isinstance(x, py_trees.composites.Parallel):
                x.add_child(sequence)
                break

        return root


class ConstructionObstacleRightLane(CustomConstructionObstacle):
    """
    Variation of ConstructionObstacle where the obstacle is in the opposite lane
    """
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=60):
    ...
    def _initialize_actors(self, config):
        """Creates all props part of the construction"""
        # self._spawn_side_prop(self._reference_waypoint)
        y_offset = get_value_parameter(config, 'y_offset', float, 0.0)

        wps = self._reference_waypoint.next(self._distance)
        if not wps:
            raise ValueError("Couldn't find a viable position to set up the construction actors")
        self._construction_wp = wps[0]
        construction_tf = self._construction_wp.transform
        construction_tf.location += construction_tf.rotation.get_right_vector() * (self._construction_wp.lane_width + 0.1 + y_offset)
        construction_tf.location.z += 0.1
        self._create_construction_setup(construction_tf, self._construction_wp.lane_width*0.9)

        self.parking_slots.append(construction_tf.location)
    ...
    def _create_behavior(self):
        ...
        root = py_trees.composites.Sequence(name="ConstructionObstacleRightLane")
        if self.route_mode:
            root.add_child(RemoveRoadLane(self._reference_waypoint.get_right_lane()))
        ...
        if self.route_mode:
            root.add_child(ReAddRoadLane(1))
        for actor in self.other_actors:
            root.add_child(ActorDestroy(actor))

        return root

class ConstructionObstacleOppositeLane(CustomConstructionObstacle):
    """
    Variation of ConstructionObstacle where the obstacle is in the opposite lane
    """
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=60):
    ...
    def _initialize_actors(self, config):
        ...
        if not wps:
            raise ValueError("Couldn't find a viable position to set up the construction actors")
        wp = wps[0]
        lane_sign = wp.lane_id > 0
        self._construction_wp = wp.get_left_lane()
        if self._construction_wp is None or self._construction_wp.lane_type != carla.LaneType.Driving:
            raise ValueError("No driveable left lane")
        if (self._construction_wp.lane_id > 0) == lane_sign:
            raise ValueError("Left lane is not opposite")

        construction_tf = self._construction_wp.transform
        self._create_construction_setup(construction_tf, self._construction_wp.lane_width)

        self._end_wp = self._move_waypoint_forward(wp, self._end_distance)
    ...
    def _create_behavior(self):
        ...
        root = py_trees.composites.Sequence(name="ConstructionObstacleOppositeLane")
        if self.route_mode:
            root.add_child(ChangeOppositeBehavior(active=False)) # TODO a bit clumsy.. how to despawn opposite vehicles?! RemoveRoadLane doesn't work
