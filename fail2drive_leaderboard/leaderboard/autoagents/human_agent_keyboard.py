...
# Palette indexed by CARLA semantic ids (CityScapes labels from the semantic camera).
...
class HumanInterface(object):
	...
	def run_interface(self, input_data):
		...
		if self.camera_idx == 0:
			image_center = self._semantic_to_cityscapes(input_data['Center_0'][1])
	...
	def _semantic_to_cityscapes(self, semantic_image):
		max_label = CITYSCAPES_PALETTE.shape[0] - 1
		# CARLA raw semantic camera encodes class ids in the R channel (index 2 in BGRA).
		labels = semantic_image[:, :, 2]
		if np.max(labels) <= max_label:
			labels = labels.astype(np.int32)
			return CITYSCAPES_PALETTE[labels]

		# If data is already colorized, convert BGR->RGB for pygame.
		return semantic_image[:, :, -2::-1]
...
class HumanAgentSteeringWheel(AutonomousAgent):
	...
	def sensors(self):
		...
		sensors = [
			...
			{'type': 'sensor.camera.semantic_segmentation', 'x': -1.5, 'y': 0.0, 'z': 2.6, 'roll': 0.0, 'pitch': -15.0, 'yaw': 0.0,
			 'width': self.camera_width, 'height': self.camera_height, 'fov': 100, 'id': 'Center_0'},
